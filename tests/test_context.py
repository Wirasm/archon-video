...
CFG = resolve(yaml.safe_load(EXAMPLE.read_text()), "social", EXAMPLE.parent, {"CARTESIA_API_KEY": "x", "PEXELS_API_KEY": "x"})
...
def test_shared_context_carries_brand_and_rules_but_not_past_openings():
    ...
    text = context_block(CFG, "# Kind: social", None, "House rules (playbook v1): R1 ...")
    assert "Brand: Acme" in text and "R1" in text
